...
class _CategoryListViewState extends State<CategoriaScreen> {
 ...
 @override
      ...
      body: FutureBuilder<List<Category>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            _mostrarSnackbarError(context, snapshot.error!);
  ...
  /*void _mostrarSnackbarError(BuildContext context, dynamic error) {
    final statusCode = (error is ApiException) ? error.statusCode : null;
    final errorInfo = ErrorHelper.getErrorDetails(statusCode);
...
Widget _buildEmptyState() {
    ...
    child: Column(
      ...
      children: [
        Text(
          ...
          style: TextStyle(color: Colors.grey[600]), // Coma añadida aquí
        ),
  ...
  void _eliminarCategoria(String id) async {
    try {
      ...
      setState(() => _futureCategories = _categoryService.fetchAllCategories());
  ...
  Widget _buildErrorWidget(Map<String, dynamic> errorInfo) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
  ...
  Widget _buildCategoryList(List<Category> categorias) {
